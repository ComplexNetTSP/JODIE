...
def evaluate(hyperparameter, data, nb_epoch=50, device="cpu", proportion_train=0.6, state=True, directory="./"):

    validation = {}
    test = {}

    for epoch in range(int(nb_epoch/2)-1,nb_epoch):
        

        print("***** Evaluate epoch : {} *****".format(epoch+1))
        ...
        if state:
            ...
            file = open(directory+"/resultats/"+data+"/resultats_"+data+"_{}_{}_{}_{}_{}_{}.txt".format(embedding_dim, learning_rate, split, lambda_u, lambda_i, epoch), "a")
            metrics = ["AUC"]
            ...
            validation[epoch+1] = [auc_val]
            test[epoch+1] = [auc_test]
        ...
        if not state:
            ...
            validation[epoch+1] = [mrr_val, recall10_val]
            test[epoch+1] = [mrr_test, recall10_test]
