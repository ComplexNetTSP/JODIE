...
class RODIE(nn.Module):
  ...
  def __init__(self, embedding_dim, num_users, num_items, num_features, activation_rnn = "tanh", MLP_hidden_layer_dim = 50):
    ...
    self.init_embedding_user = nn.Parameter(torch.Tensor(self.embedding_dim))
    self.init_embedding_item = nn.Parameter(torch.Tensor(self.embedding_dim))
    
    # initialize RNNs
    input_rnn_user_dim = self.embedding_dim + self.num_features + 1
    input_rnn_item_dim = self.embedding_dim + self.num_features + 1
    
    self.rnn_user = RNNCell(input_rnn_user_dim, self.embedding_dim, nonlinearity = self.activation_rnn)
    self.rnn_item = RNNCell(input_rnn_item_dim, self.embedding_dim, nonlinearity = self.activation_rnn)

    # differents layers (projection, prediction)
    self.layer_projection  = NormalLinear(1, self.embedding_dim)
    self.layer_prediction  = nn.Linear(self.num_users + self.num_items + self.embedding_dim * 2, self.num_items + self.embedding_dim)
    self.layer1_pred_label = nn.Linear(self.embedding_dim, self.MLP_hidden_layer_dim)
  ...
  def update_rnn_item(self, embedding_item, embedding_user, features, delta_i):
    concat_input1 = torch.cat([embedding_user, features, delta_i.reshape(-1, 1)], dim = 1)
  ...
  def update_rnn_user(self, embedding_user, embedding_item, features, delta_u):
    concat_input2 = torch.cat([embedding_item, features, delta_u.reshape(-1, 1)], dim = 1)
  ...
  def loss_predict_state(self, model, device, interaction_id, all_previous_embeddings_user, true_label, loss_function):
    proba = model.predict_state(all_previous_embeddings_user[interaction_id, :])
    y_true = Variable(torch.LongTensor(true_label).to(device)[interaction_id])
